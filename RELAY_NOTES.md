...
- Optional metaphysical reasoning (v2.1): minimal advisory **`reasoning`** when **`AXIOMURGY_REASONING=1`** (default off); **`classification.derived_keys`** contracts the minimal top-level surface (superset when experimental adds **`experimental`**); **`AXIOMURGY_REASONING_EXPERIMENTAL=1`** adds flat **`reasoning.experimental`** (correspondence, friction, combinatorics, Wyrd hints, generation); narrow attestation allowlist per subtree; **`telos`** separate from Ouroboros **`acceptance_contract`**; **`AXIOMURGY_WYRD=1`** + SQLite for **`experimental.wyrd_hints`** when experimental is on; see **`docs/REASONING_SURFACES.md`**
