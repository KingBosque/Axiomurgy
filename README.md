...
When **`AXIOMURGY_REASONING=1`**, `--describe` and `--plan` may include a small advisory **`reasoning`** object: **`classification`** (including **`derived_keys`** for the contracted top-level surface), **`governor`**, **`telos`** (`final_cause` / `objectives`), **`scene`**, **`dialectic`**, and **`habitus`** (descriptive context only: paths, not policy). Default is off so fingerprints and attestation behavior are unchanged. Phase-advanced blocks (**correspondence**, **friction**, **combinatorics_search**, **wyrd_hints**, **generation_candidates**, and with generation + **`AXIOMURGY_REASONING_LULLIAN=1`**, **candidate_verification**) appear only under **`reasoning.experimental`** when **`AXIOMURGY_REASONING_EXPERIMENTAL=1`**. **Correspondence** maps compiled plan structure to **`telos` objectives** (clusters, objective links, optional repeated pipeline motifs). **Friction** surfaces bounded heuristic fragility (scores, risk factors, bottlenecks)—not stochastic simulation. With experimental on, **`AXIOMURGY_WYRD=1`** enables **Wyrd v1**: **`--plan`** appends an operational graph snapshot to `<artifact-dir>/wyrd/graph.sqlite` (append-only); **`experimental.wyrd_hints`** surfaces bounded read-mostly summaries. Wyrd does **not** drive execution or policy. With **`AXIOMURGY_REASONING_GENERATION=1`**, **Parthenogenesis v1** fills **`generation_candidates`** with a small capped set of **review-bound, non-executable** candidate offspring (no default spell-file writes). With **`AXIOMURGY_REASONING_LULLIAN=1`** (and generation on), **Lullian v1** adds **`candidate_verification`**: bounded symbolic comparison and ranking of those candidates—**not** a search engine and not auto-selection. Allowlisted attestation paths match this minimal surface plus **`reasoning.experimental.*`** only. **`reasoning.experimental`** is a flat map (no nested maturity taxonomies).

**Side-by-side shapes and example env:** [docs/REASONING_SURFACES.md](docs/REASONING_SURFACES.md). **`telos`**, **`governor`**, and **`dialectic`** are **shadow-mode** in this phase: deterministic, plan-aware, advisory only—no execution or policy impact.

**Reasoning efficacy evaluation (offline):** compare advisory outputs across modes and a small spell corpus with `python scripts/eval_reasoning_efficacy.py` — see [docs/REASONING_EVAL_HARNESS.md](docs/REASONING_EVAL_HARNESS.md) (not execution success; not attested artifacts).
